...
dl_fsel <- df_l |> group_by(x, y) |> summarise(n = n()) |> 
	# sample 25 points
	sample_n(size = 25, weight = n, replace = TRUE) |> 
	mutate(sel = 1)

df2 <- df_l |> left_join(dl_fsel, by = c('x', 'y')) |> 
	filter(sel == 1) 
